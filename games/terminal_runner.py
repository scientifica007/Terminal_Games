...
from games.progress import BestScoreTracker, ProgressDataError, clear_save, load_state, save_state
...
FRAME_INTERVAL = 0.05
BASE_SPEED = 7.5
SPEED_STEP_MULTIPLIER = 0.25
SPEED_PER_LEVEL = BASE_SPEED * SPEED_STEP_MULTIPLIER
MAX_SPEED_MULTIPLIER = 10.0
MAX_SPEED = BASE_SPEED * MAX_SPEED_MULTIPLIER
BASE_LEVEL_DURATION = 10.0
FAST_LEVEL_THRESHOLD = 2.0
FAST_LEVEL_DURATION_GAIN = 5.0
MAX_LEVEL_DURATION = 50.0
MAX_WORLD_STEP = 0.5

JUMP_VELOCITY = 12.5
GRAVITY = 22.0
MIN_REACTION_TIME = 1.15
...
SPEED_PROFILES: dict[str, SpeedProfile] = {
    "relaxed": SpeedProfile("relaxed", "Relaxed", 1.00),
    "normal": SpeedProfile("normal", "Normal", 1.50),
    "fast": SpeedProfile("fast", "Fast", 2.00),
    "expert": SpeedProfile("expert", "Expert", 3.00),
}
SPEED_CHOICES = {"1": "relaxed", "2": "normal", "3": "fast", "4": "expert"}
...
def max_level_for_mode(speed_mode: str = DEFAULT_SPEED_MODE) -> int:
    start = BASE_SPEED * _speed_profile(speed_mode).start_multiplier
    if start >= MAX_SPEED:
        return 1
    return math.ceil((MAX_SPEED - start) / SPEED_PER_LEVEL - 1e-12) + 1


def at_speed_cap(state: GameState) -> bool:
    return current_speed(state) >= MAX_SPEED - 1e-9


def level_duration(level: int, speed_mode: str = DEFAULT_SPEED_MODE) -> float:
    """Active seconds before the next speed increase; fast levels last longer."""
...
def advance_level_clock(state: GameState, dt: float) -> None:
    ...
    if at_speed_cap(state):
        state.level = min(state.level, max_level_for_mode(state.speed_mode))
        state.level_elapsed = 0.0
        return
    state.level_elapsed += dt
    while not at_speed_cap(state):
        duration = level_duration(state.level, state.speed_mode)
        if state.level_elapsed < duration:
            break
        state.level_elapsed -= duration
        state.level += 1
    if at_speed_cap(state):
        state.level = min(state.level, max_level_for_mode(state.speed_mode))
        state.level_elapsed = 0.0
...
def horizontal_overlap(obstacle: Obstacle) -> bool:
    return PLAYER_X < obstacle.x + obstacle.spec.width and PLAYER_X + PLAYER_HITBOX_WIDTH > obstacle.x
...
def collides_with_player(state: GameState, obstacle: Obstacle) -> bool:
    ...
    return state.player_y < obstacle.spec.height and state.player_y + PLAYER_HITBOX_HEIGHT > 0.0


def detect_collision(state: GameState) -> Obstacle | None:
    return next((o for o in state.obstacles if collides_with_player(state, o)), None)


def _step_subframe(state: GameState, dt: float, rng: Random) -> None:
    ...
    state.obstacles = [o for o in state.obstacles if o.x + o.spec.width >= -1.0]
...
def step_game(state: GameState, dt: float, rng: Random) -> None:
    """Advance one rendered frame using collision-safe high-speed substeps."""
    if dt <= 0:
        raise ValueError("dt must be positive.")
    if not state.alive:
        return
    state.frame_index += 1
    remaining = dt
    while remaining > 1e-12 and state.alive:
        speed = max(current_speed(state), 1e-9)
        sub_dt = min(remaining, MAX_WORLD_STEP / speed)
        _step_subframe(state, sub_dt, rng)
        remaining -= sub_dt
...
def _nearest_new_progress(multiplier: float) -> tuple[str, int]:
    """Map an old physical speed to the closest new mode/level combination."""
    best: tuple[float, str, int] | None = None
    for mode in SPEED_PROFILES:
        for level in range(1, max_level_for_mode(mode) + 1):
            delta = abs(speed_multiplier_for_level(level, mode) - multiplier)
            candidate = (delta, mode, level)
...
def _old_v2_multiplier(speed_mode: str, level: int) -> float:
    old_starts = {"relaxed": 0.85, "normal": 1.0, "fast": 1.25, "expert": 1.5}
    if speed_mode not in old_starts:
        raise ValueError("Invalid saved speed mode.")
    return min(3.0, old_starts[speed_mode] + (level - 1) * 0.10)


def _old_v2_duration(multiplier: float) -> float:
    return min(23.0, 8.0 + max(0.0, multiplier - 1.5) * 10.0)
...
def serialize_session(state: GameState) -> dict[str, Any]:
    return {
        ...
        "obstacles": [
            {"kind": o.spec.kind, "x": o.x, "counted": o.counted}
...
def deserialize_session(payload: dict[str, Any]) -> GameState:
    ...
    if version not in {LEGACY_SAVE_VERSION, ADAPTIVE_SAVE_VERSION, SAVE_VERSION}:
        raise ValueError("Unsupported Terminal Runner save version.")
    ...
    spawn_remaining = _finite_number(payload.get("spawn_remaining"), "spawn distance", minimum=0.0)
    ...
    if version == LEGACY_SAVE_VERSION:
        ...
        old_level = legacy_score // LEGACY_LEVEL_SCORE_STEP + 1
        old_multiplier = min(2.2, 1.0 + (old_level - 1) * 0.10)
        speed_mode, level = _nearest_new_progress(old_multiplier)
        level_elapsed = 0.0
    elif version == ADAPTIVE_SAVE_VERSION:
        old_mode = payload.get("speed_mode")
        old_level = _positive_int(payload.get("level"), "level")
        if not isinstance(old_mode, str):
            raise ValueError("Invalid saved speed mode.")
        old_multiplier = _old_v2_multiplier(old_mode, old_level)
        old_elapsed = _finite_number(payload.get("level_elapsed"), "level elapsed time", minimum=0.0)
        old_duration = _old_v2_duration(old_multiplier)
        if old_elapsed >= old_duration + 1e-9:
            raise ValueError("Invalid saved level elapsed time.")
        speed_mode, level = _nearest_new_progress(old_multiplier)
        fraction = min(1.0, old_elapsed / old_duration) if old_duration else 0.0
        level_elapsed = 0.0 if speed_for_level(level, speed_mode) >= MAX_SPEED else fraction * level_duration(level, speed_mode)
    else:
        ...
        if level > max_level_for_mode(speed_mode):
            raise ValueError("Invalid saved level.")
        level_elapsed = _finite_number(payload.get("level_elapsed"), "level elapsed time", minimum=0.0)
        if at_speed_cap(GameState(speed_mode=speed_mode, level=level)):
            if level_elapsed > 1e-9:
                raise ValueError("Invalid saved level elapsed time.")
        elif level_elapsed >= level_duration(level, speed_mode) + 1e-9:
            raise ValueError("Invalid saved level elapsed time.")
    ...
    for raw in raw_obstacles:
        ...
        spec = OBSTACLE_BY_KIND.get(raw.get("kind"))
        if spec is None:
...
def render_world(state: GameState, best_score: int = 0, *, paused: bool = False, status: str = "") -> str:
    ...
    shift = int(state.distance) % len(GROUND_PATTERN)
    texture = "".join(GROUND_PATTERN[(shift + c) % len(GROUND_PATTERN)] for c in range(VIEW_WIDTH))
    ...
    top = GROUND_ROW - len(sprite) - int(round(state.player_y))
    for row_offset, sprite_row in enumerate(sprite):
        _draw_text(canvas, top + row_offset, int(PLAYER_X), sprite_row)

    profile = _speed_profile(state.speed_mode)
    multiplier = current_speed(state) / BASE_SPEED
    next_text = "MAX" if at_speed_cap(state) else f"{level_time_remaining(state):.1f}s"
    lines = [
        "=== Terminal Runner ===",
        "Space/Up: jump | P: pause | S: save | Q/Esc: quit",
        (
            f"Score: {score_for_state(state):05d}   Best: {best_score:05d}   "
            f"Mode: {profile.name}   Level: {state.level}   "
            f"Speed: {multiplier:.2f}x   Next: {next_text}   Passed: {state.obstacles_passed}"
        ),
...
def choose_speed_mode() -> str | None:
    ...
    for number, mode in SPEED_CHOICES.items():
    ...
    print("Q. Back")
    aliases = {mode: mode for mode in SPEED_PROFILES}
    while True:
        choice = input("Speed: ").strip().lower()
        if choice in {"q", "quit", "back", "exit"}:
            return None
        mode = SPEED_CHOICES.get(choice) or aliases.get(choice)
        if mode is not None:
            return mode
        print("Choose 1, 2, 3, 4, or Q.")


def play_round(rng: Random | None = None, saved: GameState | None = None, *, speed_mode: str = DEFAULT_SPEED_MODE) -> bool:
    ...
    try:
        try:
            with KeyReader() as reader:
                ...
                while state.alive:
                    print("\033[H" + render_world(state, best.best_score, paused=paused, status=status), end="", flush=True)
                    if paused:
...
def main() -> None:
    ...
    while True:
        ...
        if action == LOAD:
            ...
            if saved is None:
                continue
            print(f"\nSaved run loaded ({_speed_profile(saved.speed_mode).name}, level {saved.level}).")
        elif action == NEW:
